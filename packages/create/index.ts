...
async function main() {
  ...
  // Post-process: replace placeholder name in package.json
  const rootPkg = path.join(dest, "package.json");
  if (await pathExists(rootPkg)) {
    const txt = await Bun.file(rootPkg).text();
    const projectName = path.basename(dest);
    const updated = txt.replaceAll("dev-hq-workspace", projectName);
    await writeFile(rootPkg, updated);
